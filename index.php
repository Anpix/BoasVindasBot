...
function processMessage($message) {
    ...
    if (isset($message['text'])) {
        ...
        } else if (stripos($text, "Feliz") && ( stripos($text, "Sá") || strpos($text, "Sa")) && strpos($text, "bado") {
            if ((date( "w", $timestamp) == 6)){
                //Se for Sábado
                apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => "Feliz Sábado!")); 
            }else{
                //Se não for Sábado
                apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => "Calma! Ainda não é Sábado!")); 
            }
        }
    }
} 
